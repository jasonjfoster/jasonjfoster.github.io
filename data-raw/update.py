...
h0a0_new = pd.DataFrame({"date": h0a0_series.index.strftime("%Y-%m-%d"),
                         "oas": (h0a0_series * 100).round(2).values})
h0a0_new = h0a0_new.dropna()
h0a0_df = pd.concat([h0a0_df[h0a0_df["date"] < h0a0_new["date"].min()],
                     h0a0_new,
                     h0a0_df[h0a0_df["date"] > h0a0_new["date"].max()]],
                    ignore_index = True)
h0a0_df.to_csv("data-raw/h0a0.csv", index = False,
               float_format = "%.10g") # match R, i.e. no trailing zeros
...
spx_df = pd.concat([spx_df[spx_df["date"] < spx_new["date"].min()],
                    spx_new,
                    spx_df[spx_df["date"] > spx_new["date"].max()]],
                   ignore_index = True)
spx_df.to_csv("data-raw/spx.csv", index = False,
              float_format = "%.10g") # match R, i.e. no trailing zeros
