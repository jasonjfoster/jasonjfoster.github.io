...
```{r}
#| echo: false
sd_xts <- roll::roll_sd(overlap_xts, width, min_obs = 1) * sqrt(scale[["periods"]]) * sqrt(scale[["overlap"]])
```
...
#| fig.width: 4
#| fig.height: 3
beta_mlt <- data.table::copy(beta_dt)[ , c("type", "spot", "diff") := NULL]
...
#| fig.width: 10
#| fig.height: 3
greeks_mlt <- data.table::copy(greeks_dt)[ , c("spot", "d1", "d2") := NULL]
...
```{r}
pnl_bond <- function(duration, convexity, dy, y, t) {
...
bonds_dt <- data.table::data.table(index = zoo::index(tail(levels_xts, width)),
                                   ...
                                   dy = tail(zoo::na.locf(levels_xts[ , factor]), width),
                                   t = (seq_len(width) - 1) / scale[["periods"]])
data.table::setnames(bonds_dt, c("index", "duration", "convexity", "dy", "t"))
bonds_dt[ , dy := (dy - y) / 100, by = index]
```

```{r}
attrib_dt <- bonds_dt[ , as.list(unlist(pnl_bond(duration, convexity, dy, as.numeric(y), t))), by = index]
```
...
#| fig.width: 6
#| fig.height: 3
attrib_mlt <- data.table::melt(attrib_dt, id.vars = "index")
...
#| fig.width: 4
#| fig.height: 3
duration_mlt <- data.table::copy(duration_dt)[ , "spot" := NULL]
...
options_dt <- data.table::data.table(index = zoo::index(tail(levels_xts, width)),
                                     spot = tail(zoo::na.locf(levels_xts[ , factor]), width),
...
sim_gbm <- function(n_sim, S, mu, sigma, dt) {
  ...
  X <- sigma * sqrt(dt) * rnorm(n_sim) + (mu - 0.5 * sigma ^ 2) * dt

  result <- S * exp(cumsum(X))
...
```{r}
levels_sim <- sim_gbm(width + 1, S[1], mu[1], sqrt(sigma[1, 1]), dt)
```
