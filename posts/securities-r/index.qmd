...
yld_newton <- function(params, cash_flows) {
  ...
  while (abs(target0 - params[["target"]]) > params[["tol"]]) {
    ...
    for (i in 1:length(cash_flows)) {
      ...
      target0 <- target0 + cash_flows[i] / (1 + yld0) ^ i
      
      # derivative of present value of cash flows
      prime0 <- prime0 - (i + 1) * cash_flows[i] / (1 + yld0) ^ (i + 2)
...
```{r}
target <- 0.9839 * 1000 # present value
start <- 0.04 # coupon
cash_flows <- rep(start * 1000, 10)
