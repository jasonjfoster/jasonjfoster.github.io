...
Connect to the Bloomberg desktop API through a local session and parse the response messages into a data frame:

-   <https://www.bloomberg.com/professional/support/api-library/>
-   <https://bloomberg.github.io/blpapi-docs/>
-   Core Developer Guide (page 80): <https://data.bloomberglp.com/professional/sites/10/2017/03/BLPAPI-Core-Developer-Guide.pdf>
...
```{python}
def process_response(event, fields, start_date = None):
  ...
  for msg in event:
    ...
    if (start_date): # historical data request
...
Request a single value per security and field, i.e. current reference data such as prices and classifications:
...
Request a time series per security and field between start and end dates, i.e. historical data such as daily prices:
...
def bdh(securities, fields, start_date, end_date = None):
  ...
  while status:
    ...
    if (event.eventType() in [blpapi.Event.RESPONSE, blpapi.Event.PARTIAL_RESPONSE]):
      result = pd.concat([result, process_response(event, fields, start_date)], ignore_index = True)
