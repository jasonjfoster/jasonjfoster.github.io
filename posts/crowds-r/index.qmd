---
title: "Crowds"
author: "[Jason Foster](mailto:jason.j.foster@gmail.com)"
date: today
categories:
  - analysis
  - finance
  - r
draft: true
execute:
  freeze: false
editor: 
  mode: source
---

```{r}
# factors_r <- c("SP500") # "SP500" does not contain dividends
# factors_d <- c("DTB3")
```

```{r}
#| echo: false
# source("../helper-levels.R")
```

# Parse web

The "wisdom of crowds" is the idea that the aggregated views of many independent participants are often more accurate than the views of any single participant. To measure the views of professional asset allocators, estimate the implied asset allocations of "Tactical Allocation" mutual funds using returns-based style analysis and then compare the average allocations of the best- and worst-performing funds.

-   <https://en.wikipedia.org/wiki/Wisdom_of_the_crowd>

First, screen the mutual fund universe for the "Tactical Allocation" category using the `yfscreen` package:

```{r}
filters <- list("eq", list("categoryname", "Tactical Allocation"))
query <- yfscreen::create_query(filters)
payload <- yfscreen::create_payload("mutualfund", query, 250)
data <- yfscreen::get_data(payload)
```

Then sort the funds by net assets and remove duplicates to keep the primary share class of each fund:

```{r}
sorted_df <- data[order(
  -data[["netAssets.raw"]],
  data[["netExpenseRatio.raw"]],
  data[["firstTradeDateMilliseconds"]],
  data[["longName"]],
  data[["symbol"]]
), ]
tickers <- sorted_df[!duplicated(sorted_df[["netAssets.raw"]]), "symbol"]
```

Finally, represent the asset allocation universe using iShares ETFs that span domestic and international equities and bonds:

```{r}
allocations <- c("IVV", "IDEV", "IUSB", "IEMG", "IJH", "IAGG", "IJR")
tickers <- c(tickers, allocations)
```

# Optimization

Estimate the implied allocations of each fund by regressing fund returns on the ETF returns with weights that are fully invested and long-only:

$$
\begin{aligned}
\begin{array}{rrcl}
\displaystyle\min&\left\|y-X\mathbf{w}\right\|^{2}\\
\textrm{s.t.}&\mathbf{w}^{T}e&=&1\\
&0\leq\mathbf{w}&\leq&1\\
\end{array}
\end{aligned}
$$

-   <https://web.stanford.edu/~wfsharpe/art/sa/sa.htm>

Solve the constrained least squares problem analytically for rolling windows using the `rolloptim` package; the commented `CVXR` implementation below is the original version, kept for reference:

```{r}
# install.packages("pak")
# pak::pak("jasonjfoster/rolloptim") # roll (>= 1.1.7)
```

```{r}
#| output: false # Attaching package: 'CVXR' - The following object is masked from 'package:stats': power
# library(CVXR)
```

```{r}
# min_rss_optim <- function(x, y) {
# 
#   params <- Variable(ncol(x))
# 
#   obj <- Minimize(sum_squares(y - x %*% params))
# 
#   cons <- list(sum(params) == 1, params >= 0, params <= 1)
# 
#   prob <- Problem(obj, cons)
# 
#   psolve(prob)
#
#   result <- value(params)
# 
#   return(result)
# 
# }
```

```{r}
#| echo: false
#| output: false # pause one second after five requests
# width <- 20 * 3
source("../helper-prices.R") # run then render

overlap_xts <- roll::roll_mean(returns_xts, scale[["overlap"]], min_obs = 1)

# overlap_xts <- na.omit(overlap_xts)
# overlap_x_xts <- overlap_xts[ , factors]
overlap_x_xts <- overlap_xts[ , allocations]
# overlap_y_xts <- overlap_xts[ , !colnames(overlap_xts) %in% c(factors, allocations)]
overlap_y_xts <- overlap_xts[ , !colnames(overlap_xts) %in% allocations]
# overlap_z_xts <- overlap_xts[ , allocations]
```

Also compute the trailing one-year performance of each fund:

```{r}
performance_xts <- exp(roll::roll_sum(returns_xts, width, min_obs = 1)) - 1
```

Then estimate the implied allocations of each fund over the trailing window:

```{r}
n_rows <- nrow(overlap_xts)
result_ls <- list()
index_ls <- list()

# for (i in width:n_rows) {
for (i in n_rows) { # last window only to limit render time
    
  idx <- max(i - width + 1, 1):i
  x_subset <- zoo::coredata(overlap_x_xts[idx, ])
  y_subset <- zoo::coredata(overlap_y_xts[idx, ])
  params_ls <- list()
  tickers_ls <- list()
  performance_ls <- list()
  
  for (j in tickers[!tickers %in% allocations]) {
    
    idx <- complete.cases(x_subset, y_subset[ , j])
    x_complete <- x_subset[idx, , drop = FALSE]
    y_complete <- y_subset[idx, j]
    
    if ((nrow(x_complete) > 0) && (length(y_complete) > 0)) {
      
      xx <- roll::roll_crossprod(x_complete, x_complete, width = nrow(x_complete))
      xy <- roll::roll_crossprod(x_complete, y_complete, width = nrow(x_complete))
      
      # params <- t(min_rss_optim(x_complete, y_complete))
      params <- rolloptim::roll_min_rss(xx, xy, total = 1, lower = 0, upper = 1)[nrow(x_complete), ]
      params_ls <- append(params_ls, list(params))
      
      tickers_ls <- append(tickers_ls, list(j))
      
      performance_ls <- append(performance_ls, list(performance_xts[i, j]))
        
    }
      
  }
  
  if (length(params_ls) > 0) {
      
    result <- do.call(rbind, params_ls)
    rownames(result) <- unlist(tickers_ls)
    
    result <- cbind(result, performance = unlist(performance_ls))
    
    result_ls <- append(result_ls, list(result))
    index_ls <- append(index_ls, list(zoo::index(overlap_xts)[i]))
      
  }
  
}
```

```{r}
# cache full-history results for time-series analysis (regenerate after logic changes)
# save(result_ls, file = "result_ls.rda")
# save(index_ls, file = "index_ls.rda")
```

# Performance

```{r}
# load cached full-history results for time-series analysis
# load("result_ls.rda")
# load("index_ls.rda")
```

Bucket funds into quartiles by trailing performance, where Q1 contains the best-performing funds:

```{r}
quantile_cut <- function(x) {

  result <- cut(
    -x,
    breaks = quantile(-x, probs = c(0, 0.25, 0.5, 0.75, 1),
                      na.rm = TRUE),
    labels = c("Q1", "Q2", "Q3", "Q4"),
    include.lowest = TRUE
  )

  return(result)

}
```

Then average the implied allocations within each quartile, as well as overall, for each date:

```{r}
n_rows <- length(result_ls)
numeric_cols <- c(allocations, "performance")
score_ls <- list()

for (i in 1:n_rows) {
  
  score_df <- data.frame(result_ls[[i]])
  colnames(score_df) <- numeric_cols
  
  score_df[["date"]] <- index_ls[[i]]
  score_df[["quantile"]] <- quantile_cut(score_df[["performance"]])
  
  score_form <- as.formula(paste0("cbind(", paste(numeric_cols, collapse = ","), ") ~ date + quantile"))
  overall_form <- as.formula(paste0("cbind(", paste(numeric_cols, collapse = ","), ") ~ date"))
  
  score <- aggregate(score_form, data = score_df, FUN = function(x) mean(x, na.rm = TRUE))
  overall <- aggregate(overall_form, data = score_df, FUN = function(x) mean(x, na.rm = TRUE))
  overall[["quantile"]] <- "Overall"
  overall <- overall[ , c("date", "quantile", numeric_cols)]
  
  score <- rbind(score, overall)
  
  score_ls <- append(score_ls, list(score))
  
}
```

```{r}
score_df <- do.call(rbind, score_ls)
print(score_df)
```

```{r}
# save(score_df, file = "score_df.rda")
# score_xts <- xts::xts(score_df[score_df[["quantile"]] == "Q1", allocations],
#                       score_df[score_df[["quantile"]] == "Q1", "date"])
# plot(score_xts)
```
