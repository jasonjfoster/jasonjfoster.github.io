...
granger_test <- function(x, y, order) {
    ...
    wald <- t(R %*% beta - r) %*% solve(R %*% vcov(fit) %*% t(R)) %*% (R %*% beta - r)
...
roll_lead_lag <- function(x, y, width, order, p_value) {
    ...
    for (i in width:n_rows) {
        ...
        if ((x_y < p_value) && (y_x > p_value)) {
            x_y_status <- TRUE # x leads y 
        } else if ((x_y > p_value) && (y_x < p_value)) {
            y_x_status <- TRUE # y leads x
        }
        
        total <- data.frame("x_y" = x_y_status,
                            "y_x" = y_x_status)
        colnames(total) <- c(x_name, y_name)
