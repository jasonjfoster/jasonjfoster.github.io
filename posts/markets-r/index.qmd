...
#| fig.width: 6
#| fig.height: 3
momentum_mlt <- data.table::melt(data.table::as.data.table(momentum_xts), id.vars = "index")
...
#| fig.width: 6
#| fig.height: 3
score_mlt <- data.table::melt(data.table::as.data.table(score_xts), id.vars = "index")
...
Outliers are defined as the observations whose regression residuals fall below $Q_{1}-1.5\times IQR$ or above $Q_{3}+1.5\times IQR$:
...
#| fig.width: 6
#| fig.height: 3
outliers_mlt <- data.table::melt(data.table::as.data.table(outliers_xts), id.vars = "index")
outliers_plt <- plot_ts_decomp(score_mlt[variable != "Overall"], decomp = "Overall", title = "Score 1Y", multiple = 1) +
  ggplot2::facet_wrap(~ variable) +
  ggplot2::geom_point(
    data = outliers_mlt,
...
#| fig.width: 3
#| fig.height: 3
ellipse_plt <- plot_scatter(data.table::data.table(returns_x_xts[ , c(1, 3)]), x = "SP500", y = "DGS10",
                            title = "Return 1D (%)") +
  ggplot2::geom_point(data = data.table::data.table(ellipse_x_xts), ggplot2::aes(x = V1 * 100, y = V2 * 100))
print(ellipse_plt)
```
...
#| fig.width: 6
#| fig.height: 3
lead_lag_mlt <- data.table::melt(data.table::as.data.table(lead_lag_xts), id.vars = "index")
