...
scaled_x_df = scaler.fit_transform(scale_x_df)

pca = PCA(n_components = len(factors))
pca_x_df = pca.fit_transform(scaled_x_df)

model = LinearRegression(fit_intercept = False)
model.fit(pca_x_df, overlap_y_df)
...
def pcr_rsq(x, y, comps):
    ...
    # result = np.dot(coef, np.dot(np.dot(x.T, x), coef.T)) / np.dot(y.T, y)
