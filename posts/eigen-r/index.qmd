...
Also, a challenge of rolling PCA is to match the eigenvectors: the sign can flip and the order can change between windows.
...
# raw_df <- rolleigen::roll_eigen(overlap_xts, width, order = FALSE)[["vectors"]][ , comp, ]
...
roll_eigen2 <- function(x, width, comp) {
  ...
  for (i in width:n_rows) {
    ...
    if (i > width) {
      ...
      order <- apply(abs(cosine), 2, which.max) # assumes a one-to-one match between windows
...
```{r}
# clean_df <- rolleigen::roll_eigen(overlap_xts, width, order = TRUE)[["vectors"]][ , comp, ]
# clean_df <- xts::xts(t(clean_df), zoo::index(overlap_xts))
# colnames(clean_df) <- colnames(overlap_xts)
```
