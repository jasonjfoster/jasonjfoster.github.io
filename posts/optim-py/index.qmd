...
def rand_weights1(n_sim, n_assets):  
  ...
  rand_unif = np.random.uniform(size = (n_sim, n_assets))
  rand_unif_sum = np.sum(rand_unif, axis = 1, keepdims = True)

  result = rand_unif / rand_unif_sum
...
#| fig.width: 4
#| fig.height: 4
plot_pairs(data.table::as.data.table(reticulate::py$approach1), title = "Weight (%)")
```
...
def rand_weights2a(n_sim, n_assets, lmbda):   
  ...
  rand_exp = -np.log(1 - np.random.uniform(size = (n_sim, n_assets))) / lmbda
  rand_exp_sum = np.sum(rand_exp, axis = 1, keepdims = True)
...
#| fig.width: 4
#| fig.height: 4
plot_pairs(data.table::as.data.table(reticulate::py$approach2a), title = "Weight (%)")
```
...
def rand_weights2b(n_sim, n_assets):
  ...
  rand_exp = np.random.exponential(size = (n_sim, n_assets))
  rand_exp_sum = np.sum(rand_exp, axis = 1, keepdims = True)
...
#| fig.width: 4
#| fig.height: 4
plot_pairs(data.table::as.data.table(reticulate::py$approach2b), title = "Weight (%)")
```
...
def rand_turnover1(n_sim, n_assets, lower, upper, target):
  ...
  # result = np.random.uniform(lower, upper, size = (n_sim, n_assets))
  # result = result - result.mean(axis = 1, keepdims = True) + target / n_assets
...
#| fig.width: 4
#| fig.height: 4
plot_pairs(data.table::as.data.table(reticulate::py$approach2), title = "Weight (%)")
```
...
```{python}
returns_x_df = returns_df.dropna()[factors] # extended history (since fund inception)
