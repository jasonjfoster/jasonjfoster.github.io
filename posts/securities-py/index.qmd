...
```{python}
beta_df = pd.DataFrame([(type, y) for y in shocks],
  columns = ["type", "shock"])
...
#| fig.width: 4
#| fig.height: 3
beta_mlt <- data.table::as.data.table(reticulate::py$beta_df)[ , c("type", "spot", "diff") := NULL]
beta_mlt <- data.table::melt(beta_mlt, id.vars = "shock")
...
#| fig.width: 10
#| fig.height: 3
greeks_mlt <- data.table::as.data.table(reticulate::py$greeks_df)[ , c("spot", "d1", "d2") := NULL]
greeks_mlt <- data.table::melt(greeks_mlt, id.vars = c("type", "shock"))
greeks_plt <- plot_scen(greeks_mlt, z = "type", title = "Greeks", xlab = "Shock") +
  ggplot2::facet_wrap(~ variable, nrow = 1, scales = "free", labeller = ggplot2::labeller(variable = capitalize))
print(greeks_plt)
```
...
Total return also includes income, i.e. the yield earned over the elapsed time $y\Delta t$, in addition to the price change:

```{python}
def pnl_bond(duration, convexity, dy, y, t):
...
factor = "DGS10"
duration = 6.5
convexity = 0.65 # market convention, i.e. 65 / 100 (see duration-yield formula)
...
bond_df = pd.DataFrame({
  ...
  "t": np.arange(width) / scale["periods"]
})
```

```{python}
attrib_df = pnl_bond(bond_df["duration"], bond_df["convexity"], bond_df["dy"], y, bond_df["t"])
```
...
#| fig.width: 6
#| fig.height: 3
attrib_mlt <- data.table::as.data.table(reticulate::py$attrib_df, keep.rownames = "index")
attrib_mlt <- data.table::melt(attrib_mlt, id.vars = "index")
...
$$
\begin{aligned}
\text{Drift}&=\frac{\partial D}{\partial y}=\frac{\partial}{\partial y}\left(-\frac{1}{P}\frac{\partial P}{\partial y}\right)\\
...
#| fig.width: 4
#| fig.height: 3
duration_mlt <- data.table::as.data.table(reticulate::py$duration_df)[ , "spot" := NULL]
duration_mlt <- data.table::melt(duration_mlt, id.vars = "shock")
...
Using the greeks, the estimated change of an option price is:
...
For a given diffusion $X(t, \omega)$ driven by:
...
def sim_gbm(n_sim, S, mu, sigma, dt):
  ...
  X = sigma * np.sqrt(dt) * np.random.normal(size = n_sim) + (mu - 0.5 * sigma ** 2) * dt

  result = S * np.exp(np.cumsum(X))
...
```{python}
levels_sim = sim_gbm(width + 1, S[0], mu[0], np.sqrt(sigma[0, 0]), dt)
```

Then simulate correlated paths and compare the annualized mean and volatility of the simulated returns with the empirical estimates:
...
```{python}
# assumes interest rates follow mean-reverting process with constant volatility
```
...
To solve $V(\sigma_{n})-V=0$ for $\sigma_{n}$, use Newton's method and repeat until $\left|V(\sigma_{n})-V\right|<\varepsilon$:
...
Similarly, solve for the yield that equates the present value of cash flows to the target price, and compute duration and convexity from the first and second derivatives:
...
def implied_vol_optim(param, type, S, K, r, q, tau, target):
  ...
  result = minimize_scalar(implied_vol_obj, args = (type, S, K, r, q, tau, target),
                           method = "bounded", bounds = (0, 1))

  return np.float64(result.x)
```
...
Similarly, solve for the yield-to-maturity using optimization:
...
def yld_optim(params, cash_flows, target):
  ...
  result = minimize_scalar(yld_obj, args = (cash_flows, target),
                           method = "bounded", bounds = (0, 1))

  return np.float64(result.x) * params["freq"]
```
