...
def yld_newton(params, cash_flows):
    ...
    while (abs(target0 - params["target"]) > params["tol"]):
        ...
        for i in range(len(cash_flows)):
            ...
            target0 += cash_flows[i] / (1 + yld0) ** (i + 1)
            
            # derivative of present value of cash flows
            prime0 -= (i + 1) * cash_flows[i] / (1 + yld0) ** (i + 2)
...
```{python}
target2 = 0.9839 * 1000 # present value
start2 = 0.04 # coupon
cash_flows = [start2 * 1000] * 10
cash_flows[-1] += 1000
```
...
def yld_optim(param, cash_flows, target):
    
    result = minimize(yld_obj, param, args = (cash_flows, target))
