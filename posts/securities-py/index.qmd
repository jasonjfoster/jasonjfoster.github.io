...
greeks_df["spot"] = greeks_df["shock"].apply(level_shock, args = (S, tau, sigma))
```

## Value

For a given spot price $S$, strike price $K$, risk-free rate $r$,
annual dividend yield $q$, time-to-maturity $\tau = T - t$, and volatility $\sigma$:
...
A similar formula holds for functions of several variables $f(x_{1},\ldots,x_{n})$. This
