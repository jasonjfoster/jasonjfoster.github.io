...
Implied shocks translate a shock in one factor into consistent shocks to the remaining factors using regression betas:
...
Alternatively, scale the movement along the first principal component, i.e. $\sqrt{\lambda_{n}}\mathbf{v}_{n}$, so the shocked factor matches the target and read the implied shocks from the remaining factors:
...
Partial moments decompose the covariance matrix into concordant and discordant co-movements relative to a target, i.e. $\Sigma=\left(\text{CLPM}+\text{CUPM}\right)-\left(\text{DLPM}+\text{DUPM}\right)$:
...
```{r}
#| output: false # Registered S3 methods overwritten by 'data.table', 'htmltools', and 'htmlwidgets'
...
Exclude the co-upper partial moment to focus on downside co-movements and then compute implied shocks using the modified covariance matrix:

```{r}
# LV <- eigen(cl_pm - (dl_pm + du_pm))
# L <- LV[["values"]]
# V <- LV[["vectors"]]
# scale <- shock / (sqrt(L[comp]) * V[ , comp])[1]
# implied_shocks_pm <- scale * (sqrt(L[comp]) * V[ , comp]) * risk_sign

cov_pm <- cl_pm - (dl_pm + du_pm)
...
Bootstrap the historical returns excluding the co-upper quadrant, i.e. periods when all factors exceed the target, to stress downside co-movements:
